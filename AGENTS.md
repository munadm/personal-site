...
- **Wait for Pullfrog before merging.** Green CI is not enough. Merge only after Pullfrog has posted its review on the PR's latest commit and every finding from it has been fixed, or answered with the reason it stays: on its thread for an inline finding, or in a PR comment for one raised in the review body. A push starts a new Pullfrog run, so wait again after each one.
